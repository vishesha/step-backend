  ...
  setupDatabase()
    .then((db) => {
      try {
        ...
        const { teamName, sensorType, date, reading, options, applied } =
          req.body;

        // Insert a new row without checking for duplicates
        const insertQuery =
          "INSERT INTO cotton_soil_moisture_sensor_data (teamName, sensorType, date, reading,options,applied) VALUES (?, ?, ?, ?,?,?)";
        db.query(
          insertQuery,
          [teamName, sensorType, date, reading, options, applied],
